...
import { paginationQuerySchema, toPaginationParams, } from "../../../http/types/pagination";
/**
 * Rota para listar usuários com paginação
 * @returns
 */
function createFindAllUsersRoute(findAllUsersUserController) {
    return createAuthRoute("find-all-users").get("/find-all-users", async ({ query, request }) => {
        try {
            const locale = resolveLocale(request.headers.get("accept-language") ?? undefined);
            const messages = getMessages(locale);
            const result = await findAllUsersUserController.handle(toPaginationParams(query));
